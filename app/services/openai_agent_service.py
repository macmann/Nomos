    ...
    def _policy(self, call, state, transcript: str, intent: dict[str, Any] | None = None, force_alternative: bool = False):
        ...
        if scenario == "inactive_meter":
            if intent.get("field") == "meter_status" and intent.get("value"):
                ...
                return {"spoken_reply": "Understood. Should Nomos contact the customer for the current meter details?", "phase": "waiting_for_next_step", "extracted_updates": updates, "should_speak": True, "should_end_call": False, "reason": "meter status captured"}
            ...
            return {"spoken_reply": f"{prefix}Is that meter still active, or was it only temporary?", "phase": "waiting_for_case_result", "extracted_updates": updates, "should_speak": True, "should_end_call": False, "reason": "ask inactive meter fields"}
