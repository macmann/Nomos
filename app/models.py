...
class CallState(Base):
    ...
    hold_mode: Mapped[bool] = mapped_column(Boolean, default=False)
    last_operator_intents: Mapped[str | None] = mapped_column(String(255))
...
class CallExtraction(Base, TimestampMixin):
    ...
    meter_inactive_reason: Mapped[str | None] = mapped_column(Text); temporary_meter: Mapped[bool | None] = mapped_column(Boolean); construction_meter: Mapped[bool | None] = mapped_column(Boolean); registration_can_continue: Mapped[bool | None] = mapped_column(Boolean); customer_contact_required: Mapped[bool | None] = mapped_column(Boolean)
