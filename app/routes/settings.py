...
def page(request:Request, db:Session=Depends(get_db), admin=Depends(current_admin)):
    ss=SettingsService(db,get_settings().app_encryption_key); return templates.TemplateResponse(request, 'settings.html', {'title':'Settings','sections':SECTIONS,'tests':TESTS,'values':ss.all_masked(),'help_text':{'voice_safe_mode':'When enabled, the app receives and logs Twilio media but does not call STT, OpenAI, or TTS.', 'tts_output_format':'Default ulaw_8000 for Twilio Media Streams.', 'greeting_on_start_enabled':'Send a fixed German TTS greeting immediately after Twilio start.', 'allow_greeting_in_safe_mode':'When true, the start greeting is still sent while voice safe mode is enabled.', 'text_debug_mode':'Run STT and agent transcript flow but skip TTS audio responses.'}})
