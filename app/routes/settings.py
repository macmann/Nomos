...
def page(request:Request, db:Session=Depends(get_db), admin=Depends(current_admin)):
    ss=SettingsService(db,get_settings().app_encryption_key); return templates.TemplateResponse(request, 'settings.html', {'title':'Settings','sections':SECTIONS,'tests':TESTS,'values':ss.all_masked()})
