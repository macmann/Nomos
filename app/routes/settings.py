...
SECTIONS={'OpenAI':['openai_api_key','openai_model','openai_temperature','openai_max_turns','agent_system_prompt','extraction_prompt'],'ElevenLabs':['elevenlabs_api_key','elevenlabs_stt_model','elevenlabs_de_voice_id','elevenlabs_en_voice_id','elevenlabs_tts_model','speech_speed','number_reading_mode'],'Twilio':['twilio_account_sid','twilio_auth_token','twilio_phone_number','twilio_webhook_base_url','twilio_max_call_duration','twilio_recording_enabled'],'Voice Debugging':['voice_safe_mode','stt_enabled','agent_enabled','tts_enabled'],'Language':['default_language','outbound_default_language','auto_detect_language'],'Compliance':['ai_disclosure_required','ai_disclosure_de','ai_disclosure_en','fake_data_only'],'MCP Mail':['mcp_server_url','mcp_auth_token','mcp_connection_id','mcp_sender_email','mcp_template_de','mcp_template_en','mcp_real_email_enabled']}
...
def page(request:Request, db:Session=Depends(get_db), admin=Depends(current_admin)):
    ss=SettingsService(db,get_settings().app_encryption_key); return templates.TemplateResponse(request, 'settings.html', {'title':'Settings','sections':SECTIONS,'tests':TESTS,'values':ss.all_masked(),'help_text':{'voice_safe_mode':'When enabled, the app receives and logs Twilio media but does not call STT, OpenAI, or TTS.'}})
